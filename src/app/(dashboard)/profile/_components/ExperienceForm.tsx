...
export default function ExperienceForm() {
  ...
  const handleStartEdit = (exp: ExperienceItem) => {
    setEditingId(exp.id);
    setIsAdding(false);
    setTempTitle(exp.title);
    setTempPeriod(exp.period);
  };
  ...
  const handleSave = () => {
    ...
    if (isAdding) {
      ...
      setExperiences(
        experiences.map((exp) =>
          exp.id === editingId ? { ...exp, title: tempTitle, period: tempPeriod } : exp
        )
      );
      setEditingId(null);
    }
            ...
            {editingId !== exp.id && (
              <div className="flex gap-2">
                <button
                  ...
                  className="px-2.5 py-1 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs text-slate-700 dark:text-slate-200 flex items-center gap-1 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                >
                  <Edit2 size={12} /> Edit
                </button>
